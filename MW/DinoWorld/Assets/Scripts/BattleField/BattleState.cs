...
public class BattleState : MonoBehaviour {
    ...
    //적 스폰 포인트
    public List<Transform> spawnPoinsts = new List<Transform>();

    private void Awake()
    {
        Debug.Log( wls.MonsterCount+ "배틀넘버");
        for(int i=0; i< WorldState.instance.MonsterCount; i++)
        {
            GameObject NewMonster = Instantiate(WorldState.instance.monsterToBattle[i], spawnPoinsts[i].position, Quaternion.identity) as GameObject;
            MonstersInBattle.Add(NewMonster);
        }
    }
